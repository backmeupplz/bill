...
bot.on('callback_query', async (callbackQuery) => {
  ...
  if (approved) {
    ...
    await switchNotification(username, isAlumni, 0);
    if (!isAlumni) {
...
async function checkReminders() {
  ...
  participants_list.forEach(async (participant) => {
    ...
    const approved = !!Number(participant[14]);
    ...
    if (approved && userTime === 0) {
      return await switchNotification(participant[0], false, 0);
    }
    if (!isTimeToRemind || approved) return;
  ...
  alumni_list.forEach(async (alumni) => {
    ...
    if (alumniObj.approve && userHours === 0) {
      return await switchNotification(alumniObj.username, true, 0);
    }
    if (!isTimeToRemind || alumniObj.approve) return;
...
async function checkIfNeedsConfirmation(msg) {
  ...
  if (userType === 'participant') {
    approved = Number(user[14])
  } else {
    approved = Number(user[9])
  }

  if (approved === 1) return;

  if (userType === 'participant') {
    await switchNotification(username, false, 1);
